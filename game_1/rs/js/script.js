...
// ========================================== nav-menu ===========================================
// 1. click vào nút có id nào thì hiện phần con của nó ra + hiện backround của nó
// 2. click lại vào chính nó thì ẩn phần con của nó đi + xóa background của nó đi
// 3. click sang nút khác hoặc bên ngoài thì ẩn phần con của nó đi + xóa background của nó đi
$('.nav-child').click(async function(e){
    // lấy ra data id của thẻ cha
    let id = $(this).attr('data-id');
    // xóa hết background của các thẻ
    ...
    // await $(`.nav-child-list[id="${id}"]`).slideToggle("fast"); 

    // let display = $(`.nav-child-list[id="${id}"]`).css('display');
    // console.log(`.nav-child-list[id="${id}"]`,display);
    // if(display == 'none') {
    //         console.log('if');
    ...
    // lấy ra trạng thái display của class `.nav-child-list[id="${id}"]`
    let display = $(`.nav-child-list[id="${id}"]`).css('display');
    if(display == 'block') {
        // console.log('if');
        $(`.nav-child[data-id="${id}"]`).removeClass('bg-click');
...
$(document).click(function(){
    ...
    $('.nav-child').removeClass('bg-click'); //ở nav-menu
    $('.category-btn').removeClass('btn-focus');
    $('.category-more-wrap').removeClass('show-cate');
    $('.nav-collapses').hide(); //mobile menu
});
